...
int main() {
    ...
    if (table == nullptr) { // table doesn't exist
        ...
        for (int i = 0; i < 1000; i++) {
            catalog.insert_record(table1, {i, "record_" + std::to_string(i), i});
        }
    ...
    catalog.delete_record(table1, RID(0,4)); // id 4
    catalog.delete_record(table1, RID(1,2)); // id 137
