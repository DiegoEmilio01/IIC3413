...
class ExprPlanLessOrEquals : public ExprPlan {
public:
    std::unique_ptr<ExprPlan> lhs;

    std::unique_ptr<ExprPlan> rhs;

    ExprPlanLessOrEquals(std::unique_ptr<ExprPlan> lhs, std::unique_ptr<ExprPlan> rhs) :
        lhs(std::move(lhs)), rhs(std::move(rhs)) { }

    std::unique_ptr<ExprPlan> clone() const override {
        return std::make_unique<ExprPlanLessOrEquals>(lhs->clone(), rhs->clone());
    }
