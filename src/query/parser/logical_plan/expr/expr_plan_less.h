...
class ExprPlanLess : public ExprPlan {
public:
    std::unique_ptr<ExprPlan> lhs;

    std::unique_ptr<ExprPlan> rhs;

    ExprPlanLess(std::unique_ptr<ExprPlan> lhs, std::unique_ptr<ExprPlan> rhs) :
        lhs(std::move(lhs)), rhs(std::move(rhs)) { }

    std::unique_ptr<ExprPlan> clone() const override {
        return std::make_unique<ExprPlanLess>(lhs->clone(), rhs->clone());
    }
